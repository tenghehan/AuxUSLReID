...
class WeightAdaptiveTripletLoss(nn.Module):
	def __init__(self, margin, lambda_value):
		super(WeightAdaptiveTripletLoss, self).__init__()
		self.margin = margin
		self.lambda_value = lambda_value

	def forward(self, emb1, _, label):
		mat_dist = euclidean_dist(emb1, emb1)
		assert mat_dist.size(0) == mat_dist.size(1)
		N = mat_dist.size(0)
		mat_sim = label.expand(N, N).eq(label.expand(N, N).t()).float()
		...
		t = (triple_dist[:, 0] - triple_dist[:, 1]) + self.margin
		e = torch.exp(self.lambda_value * t)
